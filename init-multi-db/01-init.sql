...
SELECT 'CREATE DATABASE artistas'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'artistas') \gexec

SELECT 'CREATE DATABASE boletos'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'boletos') \gexec
...
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'compras') \gexec
...
SELECT 'CREATE DATABASE precios'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'precios') \gexec

SELECT 'CREATE DATABASE recintos'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'recintos') \gexec
